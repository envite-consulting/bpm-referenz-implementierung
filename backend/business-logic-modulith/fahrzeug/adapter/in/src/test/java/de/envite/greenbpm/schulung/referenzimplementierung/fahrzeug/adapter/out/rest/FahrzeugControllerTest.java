...
class FahrzeugControllerTest {
    ...
    @MockitoBean
    private FahrzeugQuery fahrzeugQueryMock;

    @MockitoBean
    private FahrzeugRestMapper fahrzeugMapperMock;

    @Test
    void should_query() throws Exception {
        final String fahrzeugId = "d370e9d6-03f6-4b4c-b71d-aba53b83f341";
        final FahrzeugResource resource = new FahrzeugResource(
                fahrzeugId,
                "Test Hersteller",
                "Test Modell",
                1901
        ...
        when(fahrzeugQueryMock.query(new FahrzeugId(fahrzeugId))).thenReturn(fahrzeugMock);
        when(fahrzeugMapperMock.toResource(fahrzeugMock)).thenReturn(resource);

        mockMvc.perform(
                get("/fahrzeug/" + fahrzeugId))
